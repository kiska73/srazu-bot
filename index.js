...
app.use(express.json());const PORT = process.env.PORT || 3000;const ALERT_FILE = "./alerts.json";function loadAlerts() {
    try {
    ...
    try {
        ...
        console.error( Error fetching ${exchange} for ${symbol}:, e.message);
    }
    return null;
    ...
    const { device_id, exchange, symbol, price, direction, tgToken, tgChatId } = req.body;if (!tgToken || !tgChatId) {
...
res.json({ ok: true });});app.post("/remove_alert", (req, res) => {
    let alerts = loadAlerts();
    const initialCount = alerts.length;alerts = alerts.filter(a => 
    !(a.device_id === req.body.device_id && a.symbol === req.body.symbol)
);

if (alerts.length !== initialCount) {
    saveAlerts(alerts);
    console.log(` Alert removed for ${req.body.symbol}`);
}
res.json({ ok: true });});setInterval(async () => {
    let alerts = loadAlerts();
    if (alerts.length === 0) return;let changed = false;
